...
...
    ...
    Status = IoCreateDevice( //一定会创建pdo的
...
/*
不会创建pdo，不会理睬传统驱动的实例
加载root下的pnp实例设备，会创建PNPROOT_DEVICE，但PNPROOT_DEVICE里面的pdo=NULL(没有创建，创建在此函数被调用)
全面搜索注册表 \\Registry\\Machine\\System\\CurrentControlSet\\Enum\\Root下的设备
（1）如果为传统设备，忽略之
（2）如果为pnp设备，那么把Instance比如0000所代表的实例要创建一个PNPROOT_DEVICE（如果不存在的话），
...
...
    ...
    //下面分配内存，包括合并其他驱动处理的relations所需的内存：
    //(1)其他驱动处理的realtions在Irp->IoStatus.Information中；
    //(2)本驱动处理知道pdo的个数=PNPROOT_DEVICE的个数
...
...
            PdoDeviceExtension = (PPNPROOT_PDO_DEVICE_EXTENSION)Device->Pdo->DeviceExtension;
            RtlZeroMemory(PdoDeviceExtension, sizeof(PNPROOT_PDO_DEVICE_EXTENSION));
            PdoDeviceExtension->Common.IsFDO = FALSE;//pdp
        ...
        ObReferenceObject(Device->Pdo);//重要
 
    ...
    IN PIRP Irp)
{
    ...
    switch (IrpSp->MinorFunction)
    {
        case IRP_MN_QUERY_DEVICE_RELATIONS:
            ...
            Status = PnpRootQueryDeviceRelations(DeviceObject, Irp);//生根
            break;
        ...
        case IRP_MN_START_DEVICE:
            ...
            IoCompleteRequest(Irp, IO_NO_INCREMENT); //最低一层了，最终必须完成
            return Status;
...
static NTSTATUS
PdoQueryDeviceRelations( //只pdo响应，很简单
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp,
...
static NTSTATUS
PdoQueryCapabilities( //仅仅修正一下传来的Capabilities
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp,
    IN PIO_STACK_LOCATION IrpSp)
{
...
//从PPNPROOT_PDO_DEVICE_EXTENSION拷贝而已，而PPNPROOT_PDO_DEVICE_EXTENSION的ResourceList信息在枚举时从注册表读取
static NTSTATUS
PdoQueryResources( 
    IN PDEVICE_OBJECT DeviceObject,//pdo
    IN PIRP Irp,
...
//从PPNPROOT_PDO_DEVICE_EXTENSION拷贝而已，而PPNPROOT_PDO_DEVICE_EXTENSION的ResourceRequirementsList信息在枚举时从注册表读取
static NTSTATUS
...
//从DeviceExtension->DeviceInfo->DeviceDescription拷贝
static NTSTATUS
PdoQueryDeviceText(
    IN PDEVICE_OBJECT DeviceObject,//pdo
    IN PIRP Irp,
    ...
    IN PIO_STACK_LOCATION IrpSp)
{
    ...
    switch (IdType)
    {
        case BusQueryDeviceID: //拷贝于DeviceExtension->DeviceInfo->DeviceID
        {
            UNICODE_STRING String;
        ...
        case BusQueryInstanceID: //拷贝于DeviceExtension->DeviceInfo->InstanceID
        {
            UNICODE_STRING String;
